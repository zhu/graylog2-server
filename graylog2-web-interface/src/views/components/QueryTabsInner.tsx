...
const adjustTabsVisibility = (maxWidth: number, setHiddenTabs, lockedTab: number) => {
  console.log('adjustTabsVisibility');
...
const queryTabs = ({ queries, selectedQueryId, titles, openTitleEditModal, lockedTab, onRemove, onSelect, setLockedTab, hiddenTabs }): QueryTabItems => {
  const navItems = [];
  const menuItems = [];
  let lockedItem = null;
...
const QueryTabsInner = ({ onRemove, onSelect, queries, selectedQueryId, titles, wrapperWidth, openTitleEditModal }:Props) => {
  ...
  const dashboardTabs = React.useMemo(() => queryTabs({
    queries,
    selectedQueryId,
    titles,
    openTitleEditModal,
  ...
  React.useLayoutEffect(() => {
    requestAnimationFrame(() => adjustTabsVisibility(maxWidth, setHiddenTabs, lockedTab));
  }, [maxWidth, setHiddenTabs, lockedTab]);
